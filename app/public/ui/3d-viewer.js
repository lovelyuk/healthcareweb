/**
 * Legacy 3D Viewer Logic — Refactored to not use imports
 */
...
function init() {
    ...
    // Use global THREE.OrbitControls if available
    const controlsCtor = THREE.OrbitControls || window.OrbitControls;
    if (controlsCtor) {
        controls = new controlsCtor(camera, canvas);
...
async function loadViewerGlb(path) {
    ...
    try {
        const loaderCtor = THREE.GLTFLoader || window.GLTFLoader;
        if (!loaderCtor) throw new Error('GLTFLoader not found');
...
window.loadViewerModel = loadViewerModel;
window.loadViewerGlb = loadViewerGlb;

// Start on DOM content loaded (since Three.js might still be loading via Script tag)
